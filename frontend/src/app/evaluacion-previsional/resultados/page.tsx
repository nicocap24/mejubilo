...
function ResultadosContent() {
	...
	const calculateResults = () => {
		...
		const fundScore = Math.min(
			100,
			Math.max(60, 60 + (saldo / 10000000) * 15 + (age / 65) * 5)
		);
		...
		>
			<div className="mt-8 bg-white rounded-3xl shadow-2xl px-6 md:px-12 py-12 md:py-16 max-w-2xl w-full flex flex-col items-center border-2 border-gray-200">
				...
				<div className="w-full space-y-8 mb-12">
							...
							<div className="space-y-3">
								<p className="text-lg">
									<span className="font-semibold text-gray-900">
										Comisión anual actual:
									</span>{' '}
									<span className="text-gray-900">
										{formatCurrency(resultados.afpRating.currentCommission)}
									</span>
								</p>
								<p className="text-lg">
									<span className="font-semibold text-gray-900">
										Ahorro potencial con AFP UNO:
									</span>{' '}
									<span className="text-gray-900">
										{formatCurrency(resultados.afpRating.potentialSavings)}
									</span>
								</p>
							</div>
						</div>
					...
					<div className="bg-gray-50 rounded-xl p-6 text-center">
						<h2 className="text-2xl font-semibold text-gray-800 mb-4">
							Calificación de tu Fondo
						</h2>
						<div
							className={`text-5xl font-bold mb-2 ${getRatingColor(
								resultados.fondoRating.score
							)}`}
						>
							{resultados.fondoRating.score}/100
						</div>
						<p className="text-lg text-gray-600">
							{resultados.fondoRating.details}
						</p>
					</div>
				</div>
					...
					<div className="space-y-4">
						<div className="bg-blue-50 rounded-xl p-6 flex justify-center items-center gap-8">
							...
							<p className="text-2xl font-bold text-blue-900">
								{formatCurrency(resultados.pensionEstimada.rentaVitalicia)}
							</p>
						</div>
						<div className="bg-green-50 rounded-xl p-6 flex justify-center items-center gap-8">
							...
							<p className="text-2xl font-bold text-green-900">
								{formatCurrency(resultados.pensionEstimada.pgu)}
							</p>
						</div>
						<div className="bg-purple-50 rounded-xl p-6 flex justify-center items-center gap-8">
							...
							<p className="text-2xl font-bold text-purple-900">
								{formatCurrency(resultados.pensionEstimada.seguroSocial)}
							</p>
						</div>
						<div className="bg-orange-50 rounded-xl p-6 flex justify-center items-center gap-8">
							...
							<p className="text-2xl font-bold text-orange-900">
								{formatCurrency(resultados.pensionEstimada.total)}
							</p>
						</div>
					</div>
				</div>
